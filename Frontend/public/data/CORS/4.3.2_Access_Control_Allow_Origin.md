...
Then the solution came. The solution was **ACAO: \***
<br><br>
...
<br><br>
But if we need to send **credential** data along with the request, the server can’t set the value of the **Access-Control-Allow-Origin** header to **\***. Because allowing both `*` and sending credentials would be a **security risk**, as it would enable any website to access **authenticated** resources from another domain. In that case, the server needs to specify the origin which is allowed to access the response. Below is an example of **Access-Control-Allow-credential:** 
<br><br>
<figure>
	<div align="center">
