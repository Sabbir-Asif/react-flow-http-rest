...
Initially, CORS designers came up with the idea of **CORS headers**. Initially, the CORS designers established rules for requests such as **GET and POST** requests. These are called **Simple Requests**. For reading responses of different domains in the browser, the concept of **origin** header in the request message and **Access-Control-Allow-Origin** header in the response message was introduced.   
...
<figure>
	<div align="center">
	<img src="/data/CORS/assets/image9.png" height="300" width="500" style='border: 2px solid black'></div>
...
This was kind of sad that following the above approach we can allow specific origins.  If we have a static public site and we want to allow it for all, then how will we allow it? Mentioning every possible origin in the response **Access-Control-Allow-Origin** header is not feasible.
...
Then the solution came. The solution was that **ACAO: \*** with no **Access-Control-Allow-Credentials**. **ACAO: \*** will only work if no cookie or credential is sent along with the request(xhr.withCredentials=False). Because allowing `*` while sending credentials would be a **security risk**, as it would enable any website to access **authenticated** resources from another domain.

**ACAO \*** is the most secure setting or one of the safest HTTP headers. Below is an example of **ACAO with \*.** 
