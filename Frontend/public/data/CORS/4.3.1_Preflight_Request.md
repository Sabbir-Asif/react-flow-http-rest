...
`Access-Control-Request-Headers` includes additional or custom headers that the request has. In response, the server includes `Access-Control-Allow-Headers` which tells us that which headers are allowed to make request with. 

<br>

`Access-Control-Expose-Headers` which indicates which response headers are visible to the client.
